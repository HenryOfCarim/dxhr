...
def setup_mesh(mesh, indices, flatvertices=None, flatnormals=None,
    ...
    for s, pm in zip(indices, poly_mats):
        s2 = frozenset(s)
        if len(s2) < 3:
            continue  # remove degenerate faces, they mess up indexing when blender removes them
        if s2 in ii:
            continue
        indices2.append(s)
...
def read_attrib(data, base, iStride, nVertices, attr_layout):
    ...
    if fmt == 2:  # float3
        values = [vec3(data, base + j*iStride + offset)
    ...
    # uchar3 unorm according to what the game tells directx, but treat it as uint (it's used for skinning indices)
    if fmt == 7:
        values = []
        for j in range(nVertices):
            x, y, z = vec3b(data, base + j*iStride + offset)
...
def read_rendermodel(context, basename, data, instanciate, armature_object=None):
    ...
    for i in range(nBatches):
        ...
        for j in range(nAttr):
            ...
            print("          Attribute #{} {:08x} {} {:08x} format={}".format(
                j, iAttrHash, vertex_attributes.get(iAttrHash, ""), iAttrLoc, iAttrFmt))
        ...
        for j in range(nLocalSubmeshes):
            ...
            print("              iStartIndex = {0} = 0x{0:x}".format(
                iStartIndex))
            print("              nTrianglesSub = {0} = 0x{0:x}".format(
                nTrianglesSub))
        ...
        if "SkinWeights" in layout and "SkinIndices" in layout:
            lSkinWeights, dimSkinWeights = read_attrib(
                data, oVertices, iStride, nVertices, layout["SkinWeights"])
            lSkinIndices, dimSkinIndices = read_attrib(
                data, oVertices, iStride, nVertices, layout["SkinIndices"])
        else:
            lSkinWeights = [(255, 0, 0)] * nVertices
            lSkinIndices = [(0, 0, 0)] * nVertices

        lVertices, dimVertices = read_attrib(
            data, oVertices, iStride, nVertices, layout["Position"])
        lNormals,  dimNormals = read_attrib(
            data, oVertices, iStride, nVertices, layout["Normal"])
        ...
        for uvmap in ("TexCoord1", "TexCoord2", "TexCoord3", "TexCoord4"):
            if uvmap in layout:
                uvValues, dimUv = read_attrib(
                    data, oVertices, iStride, nVertices, layout[uvmap])
                assert dimUv == 2
        ...
        for color in ("Color1", "Color2"):
            if color in layout:
                lAttr = [vec1f(data, oVertices + j*iStride +
                               layout[color][0] + 0)[0] for j in range(nVertices)]
                colors[color] = lAttr
                    ...
                    for j in range(3):
                        ...
                        if w:
                            if x < len(gs):
                                ...
                                group = bone_groups.get(x)
                            if group is None:
                                print("invalid skin bone index", x,
                                      "for", name, "with", nBones, "bones")
                                continue
...
def read_renderterrain(context, basename, data, instanciate):
    ...
    for i in range(nFormats):
        ...
        oFormatStream += 0x10
        oFormats.append((iStride, [struct.Struct("<LHH").unpack_from(
            data, oFormatStream + j*8) for j in range(nAttr)]))
    ...
    for i in range(nLists):
        ...
        if oList == 0 or oList >= len(data)-2:
            continue
        # TerrainChunkArray
        ...
        for j in range(nRanges):
            ...
            print("{:x}/{:x} {:x} @ {:x} = {:x} + {:x}".format(i,
                  j, target, oRange, oList, oRange-oList))
            # RenderTerrainGroup
            ...
            setup_mesh(mesh,
                       list(
                           zip(lIndices[0::3], lIndices[1::3], lIndices[2::3])),
                       flattened_xyz,
...
def read_mat(basename, matref):
    ...
    def build_node_graph():
        ...
        for i, (ps, vs, _, _, tex_ids) in enumerate(submats):
            mat["submat_{:02}".format(i)] = "ps {} vs {}".format(
                hash_or_repr(ps), hash_or_repr(vs))
            if not tex_ids:
                continue
            for j, (tex_id, binding) in enumerate(tex_ids):
                ...
                tex.label = "submat {} tex {} id {:04x} bind {}".format(
                    i, j, tex_id, binding)
                tex.location.x = -400
                tex.location.y = y
                tex.hide = True
        ...
        if len(texs) > 0:
            ...
            if primary_tex >= len(texs):
                print("out of bounds spec for ps_hash_primary_texture[0x{:04x}] = {} when #texs is {}".format(
                    ps_hash, primary_tex, len(texs)))
                primary_tex = len(texs)-1

            mat.node_tree.links.new(
                bsdf.inputs["Base Color"], texs[primary_tex].outputs["Color"])

        for tex, bsdf_input in ps_hash_further_mappings.get(ps_hash, []):
            mat.node_tree.links.new(
                bsdf.inputs[bsdf_input], texs[tex].outputs["Color"])
...
def load_mesh(out, context, sections, sec, sname, instanciate):
    ...
    if sec.typeid == 0xC:
        if sec.subtypeid == 26:
            ...
            meshes = read_rendermodel(
                context, sname, data, instanciate, armature_object)
            # except Exception as e:
...
def execute_items(context, items, instanciate=True):
    ...
    for basename, filepath in items:
        ...
        if not isinstance(filepath, str):
            ...
            load_mesh(out, context, filepath.sections,
                      filepath.section, basename, instanciate)
            continue
        ...
        if data[0:4] == b"Mesh":
            ...
            out.append(read_collisionmesh(
                context, basename, data, indices, instanciate))
...
def menu_func_import(self, context):
    self.layout.operator(CDCMeshImporter.bl_idname,
                         text="CDC Mesh (.drm/.cdcmesh)")
